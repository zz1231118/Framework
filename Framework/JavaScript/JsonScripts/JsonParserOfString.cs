...
namespace Framework.JavaScript
{
    ...
    internal static partial class JsonParser
    {
        ...
        private static decimal ReadNumber(TextReader reader)
        {
            ...
            if (!double.TryParse(dstr, out double dval))
                throw new JsonFormatException(dstr);

            return (decimal)dval;
        }

        private static JsonBinary ReadBinary(TextReader reader)
        {
            if (reader.Read() != 66)//B
                throw new JsonException("转换 JsonBinary 失败 [JsonBinary 始 匹配失败]");
            if (reader.Read() != 91)//[
                throw new JsonException("转换 JsonBinary 失败 [JsonBinary 始 匹配失败]");

            var json = new JsonBinary();
            if (reader.Peek() != 93)//]
                json.Add(Convert.ToByte(ReadNumber(reader)));
            ...
            while (reader.Peek() >= 0)
            {
                switch (reader.Read())
                {
                    ...
                    case 93://]
                        return json;
                    default:
                        throw new JsonException(string.Format("转换 JsonBinary 失败 [JsonBinary 乱码字符] [{0}]", reader.ReadToEnd()));
                }
            }

            throw new JsonException("转换 JsonBinary 失败 [JsonBinary 终 匹配失败]");
        }
